...
pattern = re.compile(r"<div\s+className=\"sticky bottom-0[^\"]*bg-surface/95[^\"]*\">\s*<Button[^>]*>\s*<Link\s+href=\{([^}]+)\}\s*>\s*Cancel\s*</Link>\s*</Button>(.*?)</div>\s*</form>", re.DOTALL)
