...
export async function GET(request: Request, { params }: { params: Promise<{ businessId: string; documentType: string; documentId: string }> }) {
  ...
  if (documentType === "purchase-order") {
    ...
    const record = getSalesQuote(businessId, session.user.id, documentId); if (!record) return NextResponse.json({ error: "Quote not found" }, { status: 404 });
    currency = record.quote.currencyCode; title = "SALES QUOTE"; number = record.quote.quoteNumber; partyName = record.customer.name; dateLabel = formatDate(record.quote.quoteDate); dueLabel = record.quote.expiryDate ? `Expiry Date: ${formatDate(record.quote.expiryDate)}` : "-"; if (currency !== access.business.currency) dueLabel += ` · Rate 1 ${currency} = ${record.quote.exchangeRateToBase} ${access.business.currency} · Base ${formatMoney(record.quote.baseTotalMinor, access.business.currency)}`; subtotalMinor = record.quote.subtotalMinor; taxMinor = record.quote.taxMinor; totalMinor = record.quote.totalMinor; rows = record.lines.map((line) => ({description: line.description, quantity: quantityMicrosToInput(line.quantityMicros), unitPrice: formatMoney(line.unitPriceMinor, currency), amount: formatMoney(line.grossAmountMinor, currency), discount: (line as any).discountType === "percentage" ? `${(line as any).discountValue}%` : ((line as any).discountType === "fixed" ? formatMoney(Number((line as any).discountValue), currency) : undefined)}));
    ...
    currency = record.order.currencyCode; title = "SALES ORDER"; number = record.order.orderNumber; partyName = record.customer.name; dateLabel = formatDate(record.order.orderDate); dueLabel = record.order.deliveryDate ? `Delivery Date: ${formatDate(record.order.deliveryDate)}` : "-"; if (currency !== access.business.currency) dueLabel += ` · Rate 1 ${currency} = ${record.order.exchangeRateToBase} ${access.business.currency} · Base ${formatMoney(record.order.baseTotalMinor, access.business.currency)}`; subtotalMinor = record.order.subtotalMinor; taxMinor = record.order.taxMinor; totalMinor = record.order.totalMinor; rows = record.lines.map((line) => ({description: line.description, quantity: quantityMicrosToInput(line.quantityMicros), unitPrice: formatMoney(line.unitPriceMinor, currency), amount: formatMoney(line.grossAmountMinor, currency), discount: (line as any).discountType === "percentage" ? `${(line as any).discountValue}%` : ((line as any).discountType === "fixed" ? formatMoney(Number((line as any).discountValue), currency) : undefined)}));
