...
import { getAccountsReceivable } from "@/modules/reports/report-service";

export default async function AccountsReceivablePage({ params }: { params: Promise<{ businessId: string }> }) {
  const { businessId } = await params;
  const { user, access } = await requireModule(businessId, "reports");
  const rows = getAccountsReceivable(businessId, user.id);
