...
export default async function CustomerStatementPage({ params, searchParams }: { params: Promise<{ businessId: string }>; searchParams: Promise<{ customerId?: string; currency?: string }> }) {
  ...
  return <div className="page-container page-wide">
    ...
    <form className="mb-4 flex flex-wrap items-end gap-3 rounded-lg border border-border bg-surface p-3"><label className="space-y-1 text-xs text-muted-foreground">Customer<select name="customerId" defaultValue={selected?.id ?? ""} className={`${selectClass} mt-1 block text-foreground`}><option value="">Choose a customer…</option>{customers.map((customer) => <option key={customer.id} value={customer.id}>{customer.name}</option>)}</select></label><label className="space-y-1 text-xs text-muted-foreground">Currency<select name="currency" defaultValue={query.currency ?? ""} className={`${selectClass} mt-1 block text-foreground`}><option value="">All currencies</option>{settings.currencies.map((currency) => <option key={currency.code} value={currency.code}>{currency.code} — {currency.name}</option>)}</select></label><Button type="submit" variant="secondary">View statement</Button></form>
