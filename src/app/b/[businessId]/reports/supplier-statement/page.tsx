...
export default async function SupplierStatementPage({ params, searchParams }: { params: Promise<{ businessId: string }>; searchParams: Promise<{ supplierId?: string; currency?: string }> }) {
  ...
  return <div className="page-container page-wide">
    ...
    <form className="mb-4 flex flex-wrap items-end gap-3 rounded-lg border border-border bg-surface p-3"><label className="space-y-1 text-xs text-muted-foreground">Supplier<SelectNative name="supplierId" defaultValue={selected?.id ?? ""} className="mt-1 block text-foreground"><option value="">Choose a supplier…</option>{suppliers.map((supplier) => <option key={supplier.id} value={supplier.id}>{supplier.name}</option>)}</SelectNative></label><label className="space-y-1 text-xs text-muted-foreground">Currency<SelectNative name="currency" defaultValue={query.currency ?? ""} className="mt-1 block text-foreground"><option value="">All currencies</option>{settings.currencies.map((currency) => <option key={currency.code} value={currency.code}>{currency.code} — {currency.name}</option>)}</SelectNative></label><Button type="submit" variant="secondary">View statement</Button></form>
