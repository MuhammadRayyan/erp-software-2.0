...
export default async function StockOnHandPage({ params, searchParams }: { params: Promise<{ businessId: string }>; searchParams: Promise<{ itemId?: string; locationId?: string; active?: string }> }) {
  ...
  return <div className="page-container"><Link href={`/b/${businessId}/reports`} className="mb-5 inline-flex items-center gap-1.5 text-[13px] text-muted-foreground hover:text-foreground"><ArrowLeft className="size-4" /> Reports</Link><div className="page-header"><div><h1 className="page-title">Stock On Hand</h1><p className="page-description">Current quantity and moving weighted-average value by location.</p></div></div>
    <form className="mb-4 flex flex-wrap items-end gap-3 rounded-lg border border-border bg-surface p-3"><label className="space-y-1 text-xs text-muted-foreground">Item<select name="itemId" defaultValue={filters.itemId ?? ""} className={`${selectClass} mt-1 block min-w-56`}><option value="">All items</option>{items.map((item) => <option key={String(item.id)} value={String(item.id)}>{String(item.sku ?? "No SKU")} · {String(item.name)}</option>)}</select></label><label className="space-y-1 text-xs text-muted-foreground">Location<select name="locationId" defaultValue={filters.locationId ?? ""} className={`${selectClass} mt-1 block min-w-48`}><option value="">All locations</option>{locations.map((location) => <option key={String(location.id)} value={String(location.id)}>{String(location.code)} · {String(location.name)}</option>)}</select></label><label className="flex h-9 items-center gap-2 rounded-md border border-border bg-surface-raised px-3 text-sm"><input type="checkbox" name="active" value="1" defaultChecked={filters.active === "1"} className="size-4 accent-[var(--primary)]" /> Active only</label><Button type="submit" variant="secondary">Apply</Button></form>
