  ...
  return (
    <div className="page-container">
      ...
      <form className="mb-4 flex flex-wrap items-end gap-3 rounded-lg border border-border bg-surface p-3">
        ...
        <label className="space-y-1 text-xs text-muted-foreground">Account<select name="accountId" defaultValue={filters.accountId ?? ""} className={`${selectClass} mt-1 block min-w-52 text-foreground`}><option value="">All accounts</option>{accounts.map((account) => <option key={account.id} value={account.id}>{account.code} {account.name}</option>)}</select></label>
        <label className="space-y-1 text-xs text-muted-foreground">Customer<select name="customerId" defaultValue={filters.customerId ?? ""} className={`${selectClass} mt-1 block min-w-48 text-foreground`}><option value="">All customers</option>{customers.map((customer) => <option key={customer.id} value={customer.id}>{customer.name}</option>)}</select></label>
