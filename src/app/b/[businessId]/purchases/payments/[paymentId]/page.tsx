...
import { getSupplierPayment } from "@/modules/supplier-payments/supplier-payment-service";
import { SupplierPaymentViewActions } from "@/modules/supplier-payments/supplier-payment-view-actions";
import { getSourceBankingStatus } from "@/modules/banking/banking-report-service";

export default async function SupplierPaymentViewPage({
  params,
}: {
  ...
  const record = getSupplierPayment(businessId, user.id, paymentId);
  if (!record) notFound();
  const banking = getSourceBankingStatus(businessId, user.id, "supplier_payment", paymentId);
  ...
  return (
    <div className="page-container page-medium">
      ...
      <article className="rounded-lg border border-border bg-surface-raised p-5 sm:p-7">
        <dl className="grid gap-4 border-b border-border pb-6 text-sm sm:grid-cols-2 lg:grid-cols-4"><div><dt className="text-xs text-muted-foreground">Supplier</dt><dd className="mt-1"><Link href={`/b/${businessId}/suppliers/${String(payment.supplier_id)}`} className="font-medium text-primary hover:underline">{String(payment.supplier_name)}</Link></dd></div><div><dt className="text-xs text-muted-foreground">Bank / Cash account</dt><dd className="mt-1">{String(payment.bank_account_code)} · {String(payment.bank_account_name)}</dd></div><div><dt className="text-xs text-muted-foreground">Reference</dt><dd className="mt-1">{String(payment.reference ?? "—")}</dd></div><div><dt className="text-xs text-muted-foreground">Description</dt><dd className="mt-1">{String(payment.description ?? "—")}</dd></div></dl>
        <section className="mt-6"><h2 className="text-sm font-semibold">Allocations</h2><div className="mt-3 overflow-x-auto rounded-md border border-border"><table className="data-table min-w-[680px]"><thead><tr><th>Purchase Invoice</th><th className="text-right!">Foreign allocated</th><th className="text-right!">Base carrying released</th><th>Status</th></tr></thead><tbody>{record.allocations.map((allocation) => <tr key={allocation.id}><td><Link href={`/b/${businessId}/purchases/invoices/${allocation.invoice_id}`} className="tabular font-medium text-primary hover:underline">{allocation.internal_number}</Link></td><td className="money text-right">{formatMoney(allocation.amount_minor, String(payment.currency_code))}</td><td className="money text-right">{formatMoney(allocation.base_carrying_amount_released, access.business.currency)}</td><td>{status === "posted" ? "Active" : "Released by reversal"}</td></tr>)}</tbody></table></div></section>
        ...
        <section className="mt-6 border-t border-border pt-5"><h2 className="text-sm font-semibold">Generated journal</h2><div className="mt-3 flex flex-wrap gap-3">{record.journals.map((journal) => <Link key={journal.id} href={`/b/${businessId}/accounting/journal/${journal.id}`} className="rounded-md border border-border bg-surface px-3 py-2 text-sm font-medium text-primary hover:underline">{journal.source_type === "supplier_payment" ? "Original" : "Reversal"}: {journal.entry_number}</Link>)}</div></section>
      </article>
    </div>
  );
}
