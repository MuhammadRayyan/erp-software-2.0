...
import { SalesOrderTable as OrderTable } from "@/modules/sales-orders/sales-order-table";
...
export default async function OrderListPage({
  ...
  // of Y".
  const orders = listSalesOrders(businessId, user.id);
  const pagination = { page: 1, pageSize: orders.length || 50, total: orders.length, totalPages: 1 };
  const customFieldColumns = orders.length
    ? listCustomFieldDefinitions(businessId, user.id, "sales_order")
        .filter((definition) => definition.showInList)
        .map(({ id, name, fieldType, selectOptions }) => ({ id, name, fieldType, selectOptions }))
    : [];
  const customValues = orders.length && customFieldColumns.length
    ? Object.fromEntries(getCustomFieldValuesForEntities(businessId, user.id, "sales_order", orders.map((order) => order.id)))
    : {};
  ...
  return (
    <div className="page-container">
      <div className="page-header"><div><h1 className="page-title">Sales Orders</h1><p className="page-description">Draft, post, collect, and inspect customer orders.</p></div><div className="flex flex-wrap gap-2"><Button asChild variant="secondary"><Link href={`/b/${businessId}/sales/receipts`}>Receipts</Link></Button><Button asChild><Link href={`/b/${businessId}/sales/orders/new`}><Plus className="size-4" /> New Order</Link></Button></div></div>
      {orders.length || pagination.total > 0 ? (
        <div className="data-panel overflow-hidden">
          <ListDateFilter pathname={`/b/${businessId}/sales/orders`} searchParams={searchParamsUrl} initialFrom={from ?? ""} initialTo={to ?? ""} fromLabel="Order from" toLabel="Order to" />
          {orders.length ? (
            <OrderTable businessId={businessId} orders={orders} customFields={customFieldColumns} customValues={customValues} serverSnapshot={columnSnapshots["sales-orders"]} />
          ) : (
          ...
          <ListPagination pathname={`/b/${businessId}/sales/orders`} searchParams={searchParamsUrl} info={paginationInfo} />
        </div>
      ) : (
        <EmptyState icon={<ReceiptText className="mx-auto mb-3 size-7 text-muted-foreground" />} title="No sales orders yet" description="Create a draft or post your first receivable." action={<Button asChild><Link href={`/b/${businessId}/sales/orders/new`}><Plus className="size-4" /> New Order</Link></Button>} />
      )}
    </div>
  );
}
