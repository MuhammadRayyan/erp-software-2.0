  ...
  return (
    <div className="page-container">
      ...
      <div className="mb-5 flex flex-col justify-between gap-4 lg:flex-row lg:items-start">
        <div><div className="flex flex-wrap items-center gap-2"><h1 className="page-title tabular">{order.orderNumber}</h1> <OrderRevisionSwitcher               businessId={businessId}               currentOrderId={order.id}               revisions={revisions}             /><StatusBadge status={order.documentStatus} /></div><p className="mt-2 text-base font-medium">{customer.name}</p><p className="mt-1 text-sm text-muted-foreground">Order date: {formatDate(order.orderDate)} — Delivery: {formatDate(order.deliveryDate)}</p><div className="mt-3 flex flex-wrap items-baseline gap-x-5 gap-y-1"><span className="money text-xl font-semibold">{formatMoney(order.totalMinor, currency)}</span><span className="text-sm text-muted-foreground">No ledger impact</span></div></div>
        <SalesOrderViewActions businessId={businessId} orderId={order.id} orderNumber={order.orderNumber} documentStatus={order.documentStatus} inventoryEnabled={access.modules.includes("inventory")} hasDeliverableItems={lines.some((line) => Boolean(line.itemId) && line.remainingMicros > 0)} emailDefaults={emailDefaults} />
      </div>
  ...
  <section aria-label="Custom fields" className="mt-8 border-t border-border pt-5">
    ...
    <dl className="mt-3 grid gap-x-8 gap-y-3 rounded-md border border-border bg-surface-muted/40 p-4 text-sm sm:grid-cols-2 lg:grid-cols-3">
      {customFieldDefinitions.map((definition) => (
        ...
        <section className="mt-8 border-t border-border pt-5"><h2 className="text-sm font-semibold">Related Invoices</h2>{record.relatedInvoices?.length ? <div className="mt-3 overflow-x-auto rounded-md border border-border"><table className="data-table min-w-[560px]"><thead><tr><th>Invoice</th><th>Status</th><th className="text-right!">Total</th></tr></thead><tbody>{record.relatedInvoices.map((invoice) => <tr key={invoice.id}><td><Link href={`/b/${businessId}/sales/invoices/${invoice.id}`} className="tabular font-medium text-primary hover:underline">{invoice.invoice_number}</Link></td><td className="capitalize">{invoice.document_status}</td><td className="money text-right">{formatMoney(invoice.total_minor, currency)}</td></tr>)}</tbody></table></div> : <p className="mt-2 text-sm text-muted-foreground">No invoices linked yet.</p>}</section>
      </article>
    </div>
  );
}
