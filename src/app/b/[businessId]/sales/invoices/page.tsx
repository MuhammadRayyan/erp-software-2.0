...
import { listInvoicesPaginated } from "@/modules/sales-invoices/invoice-service";
...
export default async function InvoiceListPage({
  ...
  // of Y".
  const { rows: invoices, ...pagination } = listInvoicesPaginated(businessId, user.id, { from, to, page, pageSize });
  const customFieldColumns = invoices.length
    ? listCustomFieldDefinitions(businessId, user.id, "sales_invoice")
        .filter((definition) => definition.showInList)
        .map(({ id, name, fieldType, selectOptions }) => ({ id, name, fieldType, selectOptions }))
    : [];
  const customValues = invoices.length && customFieldColumns.length
    ? Object.fromEntries(getCustomFieldValuesForEntities(businessId, user.id, "sales_invoice", invoices.map((invoice) => invoice.id)))
    : {};
    ...
    <div className="page-container">
      ...
      {invoices.length || pagination.total > 0 ? (
        <div className="data-panel overflow-hidden">
          <ListDateFilter pathname={`/b/${businessId}/sales/invoices`} searchParams={searchParamsUrl} initialFrom={from ?? ""} initialTo={to ?? ""} fromLabel="Invoice from" toLabel="Invoice to" />
          {invoices.length ? (
            <InvoiceTable businessId={businessId} invoices={invoices} customFields={customFieldColumns} customValues={customValues} serverSnapshot={columnSnapshots["sales-invoices"]} />
          ) : (
          ...
          <ListPagination pathname={`/b/${businessId}/sales/invoices`} searchParams={searchParamsUrl} info={paginationInfo} />
        </div>
      ) : (
        <EmptyState icon={<ReceiptText className="mx-auto mb-3 size-7 text-muted-foreground" />} title="No sales invoices yet" description="Create a draft or post your first receivable." action={<Button asChild><Link href={`/b/${businessId}/sales/invoices/new`}><Plus className="size-4" /> New Invoice</Link></Button>} />
      )}
    </div>
  );
}
