...
import { EQuoteSourcePanel } from "@/modules/einvoicing/source-panel";
...
export default async function QuoteViewPage({ params, searchParams }: { params: Promise<{ businessId: string; quoteId: string }>; searchParams: Promise<{ notice?: string }> }) {
  ...
  const eQuote = quote.documentStatus === "posted" ? getEQuoteForSource(businessId, user.id, "sales_quote" as any, quoteId) : null;
  const eQuoteLocked = Boolean(eQuote && ["Submitted", "Accepted", "Rejected"].includes(eQuote.status));
