...
export function TaxSettingsForm({ businessId, initial, isAdmin }: { businessId: string; initial: TaxSettingsInput; isAdmin: boolean }) {
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-5 md:grid-cols-2">
      ...
      <div className="space-y-1.5"><Label htmlFor="default-emirate">Default supply Emirate</Label><select id="default-emirate" disabled={!isAdmin} className={selectClass} value={values.defaultSupplyEmirate ?? ""} onChange={(event) => setValues((current) => ({ ...current, defaultSupplyEmirate: event.target.value as Emirate }))}><option value="">Choose Emirate</option>{emirates.map((emirate) => <option key={emirate} value={emirate}>{emirateLabels[emirate]}</option>)}</select><p className="text-xs text-muted-foreground">Document-level review can override this default.</p></div>
