...
export function InvoiceForm({
  ...
  return (
    <form className="space-y-5 max-w-4xl" noValidate>
      ...
      <section className="form-section">
        <div className="mb-3 flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-base font-semibold">Line items</h2><p className="mt-1 text-sm text-muted-foreground">Select an item for inventory sales, or leave it blank for a service line.</p></div><div className="flex gap-2"><Button type="button" variant="ghost" size="sm" onClick={() => setShowLineProjects((value) => !value)} aria-pressed={showLineProjects}><Columns3 className="size-4" /> {showLineProjects ? "Hide line Projects" : "Show Project per line"}</Button></div></div>
        {typeof errors.lines?.message === "string" && <p className="field-error mb-2">{errors.lines.message}</p>}
        <div className="overflow-x-auto pb-4 -mx-2 px-2">
          <table className={`line-item-table ${showLineProjects ? "min-w-[1480px]" : "min-w-[1270px]"}`}>
            ...
            <tbody>{fields.map((field, index) => <tr key={field.id} ><td><SelectNative  aria-label={`Line ${index + 1} inventory item`} {...register(`lines.${index}.itemId`, { onChange: (event) => selectItem(index, event.target.value) })}><option value="">Service / free text</option>{items.map((item) => <option key={item.id} value={item.id}>{item.sku ? `${item.sku} — ` : ""}{item.name}</option>)}</SelectNative></td><td className="align-top"><Input aria-label={`Line ${index + 1} description`} {...register(`lines.${index}.description`)} />{errors.lines?.[index]?.description && <p className="field-error">{errors.lines[index]?.description?.message}</p>}</td><td className="align-top"><Input className="money text-right" type="number" step="0.0001" min="0.0001" aria-label={`Line ${index + 1} quantity`} {...register(`lines.${index}.quantity`)} /></td><td className="align-top"><Input className="money text-right" type="number" step="0.000001" min="0" aria-label={`Line ${index + 1} rate`} {...register(`lines.${index}.unitPrice`)} /></td><td className="align-top">{lines[index]?.itemId ? <><input type="hidden" {...register(`lines.${index}.salesAccountId`)} /><span className="text-sm text-muted-foreground">From item</span></> : <SelectNative  aria-label={`Line ${index + 1} sales account`} {...register(`lines.${index}.salesAccountId`)}>{salesAccounts.map((account) => <option key={account.id} value={account.id}>{account.code} {account.name}</option>)}</SelectNative>}</td><td className="align-top"><SelectNative  aria-label={`Line ${index + 1} tax code`} {...register(`lines.${index}.taxCodeId`)}>{taxCodes.map((taxCode) => <option key={taxCode.id} value={taxCode.id}>{taxCode.name}</option>)}</SelectNative></td>{showLineProjects && <td className="align-top"><SelectNative  aria-label={`Line ${index + 1} project override`} {...register(`lines.${index}.projectId`)}><option value="">Use document Project</option>{availableProjects.map((project) => <option key={project.id} value={project.id}>{project.code} — {project.name}</option>)}</SelectNative></td>}<td className="money text-right">{formatMoney(previews[index]?.grossMinor ?? 0, currencyCode, minorUnit)}</td><td><Button type="button" variant="ghost" size="icon" aria-label={`Remove line ${index + 1}`} disabled={fields.length === 1} title={fields.length === 1 ? "An invoice needs at least one line" : undefined} onClick={() => remove(index)}><Trash2 className="size-4 text-muted-foreground" /></Button></td></tr>)}</tbody>
          </table>
          </div>
          <button type="button" className="dashed-add-button mt-2" onClick={() => append({ itemId: "", description: "", quantity: "1", unitPrice: "0.00", salesAccountId: defaultSalesAccountId, taxCodeId: defaultTaxCodeId, projectId: "" })}>
