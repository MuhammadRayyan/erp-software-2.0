    ...
    <form className="space-y-7" noValidate>
      ...
      <section className="border-b border-border pb-7">
        ...
        <div className="mt-5 grid gap-5 md:grid-cols-3">
          ...
          <div className="space-y-1.5"><Label htmlFor="supplyEmirate">Supply Emirate</Label><select id="supplyEmirate" className={selectClass} {...register("supplyEmirate")}><option value="">Use business default</option>{emirates.map((emirate) => <option key={emirate} value={emirate}>{emirateLabels[emirate]}</option>)}</select><p className="text-xs text-muted-foreground">Reviewed for standard-rated Sales reporting.</p></div>
          <div className="space-y-1.5"><Label htmlFor="projectId">Project <span className="font-normal text-muted-foreground">(optional)</span></Label><select id="projectId" className={selectClass} {...register("projectId")} aria-invalid={!!errors.projectId}><option value="">No project</option>{availableProjects.map((project) => <option key={project.id} value={project.id}>{project.code} — {project.name}</option>)}</select>{errors.projectId && <p className="field-error">{errors.projectId.message}</p>}</div>
      ...
      <section>
        ...
        <div className="overflow-x-auto rounded-lg border border-border bg-surface-raised">
          <table className={`data-table ${showLineProjects ? "min-w-[1480px]" : "min-w-[1270px]"}`}>
            ...
            <tbody>{fields.map((field, index) => <tr key={field.id} className="hover:bg-transparent!"><td><select className={selectClass} aria-label={`Line ${index + 1} inventory item`} {...register(`lines.${index}.itemId`, { onChange: (event) => selectItem(index, event.target.value) })}><option value="">Service / free text</option>{items.map((item) => <option key={item.id} value={item.id}>{item.sku ? `${item.sku} — ` : ""}{item.name}</option>)}</select></td><td className="py-2"><Input aria-label={`Line ${index + 1} description`} {...register(`lines.${index}.description`)} />{errors.lines?.[index]?.description && <p className="field-error">{errors.lines[index]?.description?.message}</p>}</td><td className="py-2"><Input className="money text-right" type="number" step="0.0001" min="0.0001" aria-label={`Line ${index + 1} quantity`} {...register(`lines.${index}.quantity`)} /></td><td className="py-2"><Input className="money text-right" type="number" step="0.000001" min="0" aria-label={`Line ${index + 1} rate`} {...register(`lines.${index}.unitPrice`)} /></td><td className="py-2">{lines[index]?.itemId ? <><input type="hidden" {...register(`lines.${index}.salesAccountId`)} /><span className="text-sm text-muted-foreground">From item</span></> : <select className={selectClass} aria-label={`Line ${index + 1} sales account`} {...register(`lines.${index}.salesAccountId`)}>{salesAccounts.map((account) => <option key={account.id} value={account.id}>{account.code} {account.name}</option>)}</select>}</td><td className="py-2"><select className={selectClass} aria-label={`Line ${index + 1} tax code`} {...register(`lines.${index}.taxCodeId`)}>{taxCodes.map((taxCode) => <option key={taxCode.id} value={taxCode.id}>{taxCode.name}</option>)}</select></td>{showLineProjects && <td className="py-2"><select className={selectClass} aria-label={`Line ${index + 1} project override`} {...register(`lines.${index}.projectId`)}><option value="">Use document Project</option>{availableProjects.map((project) => <option key={project.id} value={project.id}>{project.code} — {project.name}</option>)}</select></td>}<td className="money text-right">{formatMoney(previews[index]?.grossMinor ?? 0, currencyCode, minorUnit)}</td><td><Button type="button" variant="ghost" size="icon" aria-label={`Remove line ${index + 1}`} disabled={fields.length === 1} title={fields.length === 1 ? "An invoice needs at least one line" : undefined} onClick={() => remove(index)}><Trash2 className="size-4 text-muted-foreground" /></Button></td></tr>)}</tbody>
          </table>
        </div>
