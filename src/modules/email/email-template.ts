 ...
 */
export function defaultDocumentSubject(ctx: DocumentEmailContext): string {
  return `${ctx.documentName} ${ctx.documentNumber} from ${ctx.businessName}`;
}

export function defaultInvoiceSubject(ctx: DocumentEmailContext): string {
  return `${ctx.documentName} ${ctx.documentNumber} from ${ctx.businessName}`;
}
 ...
 */
export function renderDocumentEmailBody(ctx: DocumentEmailContext): string {
  ...
  const attachmentNote = ctx.hasPdfAttachment
    ? `<p style="margin:0 0 12px;font-size:13px;color:#475569;">A copy of <strong>${escapeHtml(ctx.documentNumber)}.pdf</strong> is attached for your records.</p>`
    : "";
  return `<!doctype html>
<html lang="en">
    ...
    <tr><td align="center">
      <table role="presentation" cellpadding="0" cellspacing="0" width="560" style="max-width:560px;background:#ffffff;border:1px solid #e2e8f0;border-radius:10px;overflow:hidden;">
        <tr><td style="padding:20px 28px;background:#0f172a;color:#ffffff;">
          <p style="margin:0;font-size:12px;letter-spacing:0.08em;text-transform:uppercase;color:#94a3b8;">${escapeHtml(ctx.documentName)}</p>
          <p style="margin:4px 0 0;font-size:20px;font-weight:600;">${escapeHtml(ctx.documentNumber)}</p>
        ...
        <tr><td style="padding:24px 28px;">
          ...
          <p style="margin:0 0 16px;font-size:15px;line-height:1.5;color:#334155;">Please find your ${escapeHtml(ctx.documentName.toLowerCase())} below.${ctx.dueDate && ctx.dueDate !== "-" && !ctx.dueDate.startsWith("Expiry Date:") && !ctx.dueDate.startsWith("Delivery Date:") ? ` Payment is due by <strong>${escapeHtml(ctx.dueDate)}</strong>.` : ""}</p>
          ${attachmentNote}
