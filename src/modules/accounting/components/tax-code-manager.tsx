      ...
      <DialogRoot open={editing !== undefined} onOpenChange={(isOpen) => !isOpen && setEditing(undefined)}>
        <DialogContent>
          ...
          <div className="mt-5 space-y-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2"><div className="space-y-1.5"><Label htmlFor="tax-direction">Direction</Label><select id="tax-direction" className={selectClass} value={values.direction} onChange={(event) => setValues((current) => ({ ...current, direction: event.target.value as TaxDirection }))}><option value="sales">Sales</option><option value="purchases">Purchases</option><option value="both">Both</option></select></div><div className="space-y-1.5"><Label htmlFor="vat-category">VAT category</Label><select id="vat-category" className={selectClass} value={values.vatCategory} onChange={(event) => setValues((current) => ({ ...current, vatCategory: event.target.value as VatCategory }))}><option value="standard">Standard</option><option value="zero_rated">Zero Rated</option><option value="exempt">Exempt</option><option value="out_of_scope">Out of Scope</option><option value="reverse_charge">Reverse Charge</option><option value="import">Import</option></select></div></div>
            <div className="space-y-1.5"><Label htmlFor="tax-account">Sales tax account</Label><select id="tax-account" className={selectClass} value={values.salesTaxAccountId} onChange={(event) => setValues((current) => ({ ...current, salesTaxAccountId: event.target.value }))}><option value="">Not required for 0%</option>{liabilityAccounts.map((account) => <option key={account.id} value={account.id}>{account.code} {account.name}</option>)}</select></div>
            <div className="space-y-1.5"><Label htmlFor="purchase-tax-account">Purchase tax account</Label><select id="purchase-tax-account" className={selectClass} value={values.purchaseTaxAccountId} onChange={(event) => setValues((current) => ({ ...current, purchaseTaxAccountId: event.target.value }))}><option value="">Not required for 0%</option>{assetAccounts.map((account) => <option key={account.id} value={account.id}>{account.code} {account.name}</option>)}</select></div>
