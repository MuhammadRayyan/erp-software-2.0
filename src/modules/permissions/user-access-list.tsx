...
export function UserAccessList({ businessId, users }: { businessId: string; users: UserAccess[] }) {
  ...
  return <><div className="mb-4 flex justify-end"><DialogRoot open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button><Plus className="size-4" /> Add local user</Button></DialogTrigger><DialogContent><DialogTitle>Add an existing local user</DialogTitle><DialogDescription>Add an existing local user by email. Invitation emails are not available.</DialogDescription><div className="mt-5 space-y-1.5"><Label htmlFor="user-email">Email</Label><Input id="user-email" type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="standard@demo.local" /></div>{error && <FormError message={error} />}<div className="mt-5 flex justify-end gap-2"><Button variant="ghost" onClick={() => setOpen(false)}>Cancel</Button><Button onClick={add}>Add user</Button></div></DialogContent></DialogRoot></div><div className="space-y-3">{users.map((entry) => <AccessRow key={entry.membershipId} businessId={businessId} entry={entry} />)}</div><div className="mt-5 flex items-start gap-3 rounded-lg border border-info/20 bg-info/5 p-4 text-sm"><ShieldCheck className="mt-0.5 size-4 shrink-0 text-info" /><p className="text-muted-foreground">Unavailable modules disappear from navigation. Server-side route checks use the same membership, so a hidden module cannot be opened directly.</p></div></>;
}
