...
export function OrderRevisionSwitcher({
  businessId,
  currentOrderId,
  revisions,
}: {
  ...
  revisions: OrderRevisionItem[];
}) {
  const current = revisions.find((r) => r.id === currentOrderId);
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start" className="w-80 p-1.5">
        ...
        <div className="max-h-72 space-y-1 overflow-y-auto">
          {revisions.map((rev) => {
            const isSelected = rev.id === currentOrderId;
            return (
              <DropdownMenuItem key={rev.id} asChild>
                <Link
                  href={`/b/${businessId}/purchases/orders/${rev.id}`}
                ...
                >
                  <div className="flex flex-col gap-0.5">
                    ...
                    <span className="text-[11px] text-muted-foreground">
                      {rev.order_date} · {formatMoney(rev.total_minor, rev.currency_code)}
                    </span>
                  </div>
