...
export function SalesQuoteTable({
  ...
  const rows = useMemo(
    ...
    [quotes, projectId, query, status, customerId],
  );
  const clearFilters = () => {
    setQuery("");
    setStatus("");
    ...
    <>
      ...
      {rows.length ? (
            ...
            <tbody>
              {rows.map((quote) => (
                <tr key={quote.id}>
                  ...
                  {columns.status && <td><StatusBadge status={quote.documentStatus} /></td>}
                  <td>
                    <Button asChild variant="ghost" size="icon">
                      <Link href={`/b/${businessId}/sales/quotes/${quote.id}`} aria-label={`Open ${quote.quote_number}`}>
        ...
        <div className="py-10 text-center">
          <p className="font-medium">No sales quotes match</p>
