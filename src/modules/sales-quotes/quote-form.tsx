...
import { saveSalesQuoteAction } from "./actions";
...
function previewLine(line: SalesQuoteInput["lines"][number] | undefined, taxCodes: TaxOption[], minorUnit: number, amountsIncludeTax: boolean) {
  try {
...
export function SalesQuoteForm({ businessId, quoteId, status = "draft", customers, salesAccounts, taxCodes, projects, items, currency, currencies, rates, initial }: { businessId: string; quoteId?: string; status?: SalesQuoteStatus; customers: Option[]; salesAccounts: AccountOption[]; taxCodes: TaxOption[]; projects: ProjectOption[]; items: ItemOption[]; currency: string; currencies: DocumentCurrencyOption[]; rates: DocumentRateOption[]; initial: SalesQuoteInput }) {
  ...
  const form = useForm<SalesQuoteInput>({ resolver: zodResolver(salesQuoteInputSchema), defaultValues: initial });
  const { register, control, handleSubmit, setError, setValue, formState: { errors, isSubmitting } } = form;
  const { fields, append, remove } = useFieldArray({ control, name: "lines" });
  const lines = useWatch({ control, name: "lines" }) ?? [];
  const currencyCode = useWatch({ control, name: "currencyCode" }) || currency; const exchangeRateToBase = useWatch({ control, name: "exchangeRateToBase" }) || ""; const exchangeRateDate = useWatch({ control, name: "exchangeRateDate" }) || ""; const exchangeRateSource = useWatch({ control, name: "exchangeRateSource" }) || ""; const quoteDate = useWatch({ control, name: "date" }) || "";
  ...
  async function save(values: SalesQuoteInput, intent: "draft" | "issue") {
    setServerError("");
    const result = await saveSalesQuoteAction(businessId, quoteId ?? null, values, intent);
  ...
  return <form className="space-y-5 max-w-none" noValidate>
    {serverError && <FormError message={serverError} />}
    <section className="form-section"><h2 className="text-base font-semibold">Quote details</h2><p className="mt-1 text-sm text-muted-foreground">Sales quotes are operational documents and never post to the ledger.</p><div className="mt-5 grid gap-5 md:grid-cols-3">
    ...
    <DocumentFormFooter onCancel={() => router.push(cancelHref)}>{status === "sent" ? <Button type="button" disabled={isSubmitting} onClick={handleSubmit((values) => save(values, "issue"))}>{isSubmitting && <LoaderCircle className="size-4 animate-spin" />} Update Issued Quote</Button> : <div className="flex gap-2"><Button type="button" variant="secondary" disabled={isSubmitting} onClick={handleSubmit((values) => save(values, "draft"))}>Save Draft</Button><Button type="button" disabled={isSubmitting} onClick={handleSubmit((values) => save(values, "issue"))}>{isSubmitting && <LoaderCircle className="size-4 animate-spin" />} Issue Quote</Button></div>}</DocumentFormFooter>
    </form>;
}
