...
export function ClassicPurchaseOrderDocument({ data, settings }: { data: InvoiceTemplateData; settings: TemplateSettings }) {
    ...
    <Document>
      <Page size="A4" style={[styles.page, { fontFamily: settings.fontName }]}>
        <View style={styles.header}>
          ...
          <View style={styles.boxedDetails}>
            <Text style={{ fontSize: 18, fontWeight: "heavy", color: primaryColor, marginBottom: 8, textAlign: "center" }}>
              {data.invoiceTitle || "PURCHASE ORDER"}
            </Text>
            <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 4 }}>
              <Text style={{ fontSize: 9, fontWeight: "heavy" }}>PO No:</Text>
            ...
            {data.dueDate && (
              <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
                <Text style={{ fontSize: 9, fontWeight: "heavy" }}>Delivery Date:</Text>
        ...
        <View style={{ marginBottom: 20 }}>
          <Text style={styles.sectionTitle}>{data.customerLabel || "Supplier"}</Text>
