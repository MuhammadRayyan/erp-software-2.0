  ...
  return <form className="space-y-7" noValidate onSubmit={handleSubmit(save)}>
    ...
    <section className="border-b border-border pb-7">
      ...
      <div className="mt-5 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
        ...
        <div className="space-y-1.5"><Label htmlFor="customerId">Customer <span className="font-normal text-muted-foreground">(optional)</span></Label><select id="customerId" className={selectClass} {...register("customerId")} aria-invalid={!!errors.customerId}><option value="">No customer</option>{customers.map((customer) => <option key={customer.id} value={customer.id}>{customer.name}</option>)}</select>{errors.customerId && <p className="field-error">{errors.customerId.message}</p>}</div>
        <div className="space-y-1.5"><Label htmlFor="status">Status</Label><select id="status" className={selectClass} {...register("status")}><option value="draft">Draft</option><option value="active">Active</option><option value="on_hold">On Hold</option><option value="completed">Completed</option><option value="cancelled">Cancelled</option></select></div>
      ...
      <div className="mt-5 space-y-1.5"><Label htmlFor="description">Description <span className="font-normal text-muted-foreground">(optional)</span></Label><textarea id="description" rows={4} className={`${selectClass} h-auto min-h-24 py-2`} {...register("description")} /></div>
    </section>
    <section className="border-b border-border pb-7">
