...
type Row = { id: string; order_number: string; customer_id: string; customer_name: string; date: string; expected_date: string | null; total_minor: number; currency_code: string; currency_minor_unit: number; documentStatus: SalesOrderStatus; projectIds: string[]; projectNames: string[] };

export function SalesOrderTable({ businessId, orders }: { businessId: string; orders: Row[] }) {
  ...
  const customerOptions = useMemo(
    () => Array.from(new Map(orders.map((order) => [order.customer_id, order.customer_name])).entries()).sort((a, b) => a[1].localeCompare(b[1])),
    [orders],
  );
  ...
  const rows = useMemo(() => orders.filter((order) => (
    ...
    && `${order.order_number} ${order.customer_name}`.toLowerCase().includes(query.trim().toLowerCase())
  )), [fromDate, orders, projectId, query, status, customerId, toDate]);
  ...
  return <>
    ...
    {rows.length ? <div className="data-panel overflow-x-auto"><table className="data-table min-w-[780px]"><thead><tr><th>Order</th><th>Customer</th><th>Date</th><th>Expected</th><th className="text-right!">Total</th><th>Status</th></tr></thead><tbody>{rows.map((order) => <tr key={order.id}><td><Link className="tabular font-medium text-primary hover:underline" href={`/b/${businessId}/purchases/orders/${order.id}`}>{order.order_number}</Link></td><td>{order.customer_name}</td><td>{formatDate(order.date)}</td><td>{order.expected_date ? formatDate(order.expected_date) : "—"}</td><td className="money text-right">{formatMoney(order.total_minor, order.currency_code, order.currency_minor_unit)}</td><td><StatusBadge status={order.documentStatus} /></td></tr>)}</tbody></table></div> : <div className="rounded-lg border border-border bg-surface py-10 text-center"><p className="font-medium">No purchase orders match</p><p className="mt-1 text-sm text-muted-foreground">Adjust the search or filters.</p><Button variant="ghost" className="mt-2" onClick={clearFilters}>Clear filters</Button></div>}
  </>;
}
