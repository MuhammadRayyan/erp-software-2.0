...
import { savePurchaseQuoteAction } from "./actions";
...
function previewLine(line: PurchaseQuoteInput["lines"][number] | undefined, taxCodes: TaxOption[], minorUnit: number, amountsIncludeTax: boolean) {
  try {
...
export function PurchaseQuoteForm({
  businessId,
  quoteId,
  status = "draft",
  suppliers,
  ...
  const [globalTaxCodeId, setGlobalTaxCodeId] = useState(
    () => initial.lines[0]?.taxCodeId || taxCodes.find((taxCode) => taxCode.rateBasisPoints === 500)?.id || taxCodes[0]?.id || "",
  );

  const form = useForm<PurchaseQuoteInput>({
    resolver: zodResolver(purchaseQuoteInputSchema),
  ...
  const quoteDate = useWatch({ control, name: "date" }) || "";
  ...
  function selectItem(index: number, itemId: string) {
    ...
    if (item.purchasePriceMinor != null) {
      form.setValue(
        ...
        formatMoney(item.purchasePriceMinor, currency, minorUnit).replace(/[^0-9.]/g, ""),
      );
    }
  ...
  async function save(values: PurchaseQuoteInput, intent: "draft" | "issue") {
    setServerError("");
    const result = await savePurchaseQuoteAction(businessId, quoteId ?? null, values, intent);
    if (result.fieldErrors) {
      for (const [field, messages] of Object.entries(result.fieldErrors)) {
        setError(field as keyof PurchaseQuoteInput, { message: messages[0] });
      }
    }
    ...
    <form className="space-y-7 max-w-none" noValidate>
        ...
        <div className="mt-5 grid gap-5 md:grid-cols-3">
          <div className="space-y-1.5">
            ...
            <SelectNative
              ...
              {...register("supplierId", {
                onChange: (event) => {
                  if (status === "draft") {
                    const code = suppliers.find((s) => s.id === event.target.value)?.defaultCurrencyCode ?? currency;
                    setValue("currencyCode", code);
                    setValue("exchangeRateToBase", code === currency ? "1" : "");
                    setValue("exchangeRateDate", code === currency ? quoteDate : "");
                    setValue("exchangeRateSource", code === currency ? "Base" : "");
                  }
                },
              })}
          ...
          <div className="space-y-1.5">
            <Label htmlFor="reference">
              Supplier RFQ / Reference <span className="font-normal text-muted-foreground">(optional)</span>
            </Label>
      ...
      <section className="border-b border-border pb-7">
        ...
        <DocumentCurrencyFields
          ...
          relevantDate={quoteDate}
          disabled={status === "sent"}
          onChange={(field, value) => setValue(field, value)}
        />
        <p className="mt-3 text-xs text-muted-foreground">
          This is an operational commitment snapshot only. A later Purchase Invoice uses its own posting and VAT rate.
        </p>
      </section>
      ...
      <section>
        <div className="mb-3 flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Select an inventory item or enter free-text service details.
            </p>
          </div>
          <div className="flex gap-4 items-center">
            <div className="flex items-center gap-2">
              <label htmlFor="globalTax" className="text-sm font-medium">
                Default Tax:
              </label>
              <select
                id="globalTax"
                className="w-40 flex h-9 items-center justify-between rounded-md border border-input bg-transparent px-3 py-2 text-sm shadow-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring disabled:cursor-not-allowed disabled:opacity-50"
          ...
          <table className="data-table w-full whitespace-nowrap min-w-max">
            <thead>
              <tr>
                ...
                <th className="w-56 min-w-[220px]">
                  Item <span className="font-normal text-muted-foreground">(optional)</span>
                </th>
                ...
                <tr key={field.id} className="hover:bg-transparent!">
                    ...
                    <td className="py-2">
                      <div className="flex gap-1 justify-end">
                        <select
                          ...
                          className="w-[70px] px-2 py-1 h-9 flex items-center justify-between rounded-md border border-input bg-transparent text-sm shadow-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring disabled:cursor-not-allowed disabled:opacity-50"
                        ...
                        {lines[index]?.discountType !== "none" && (
                          <Input
                            ...
                            className="money w-20 text-right px-2 py-1 h-9"
                            type="number"
                            step="any"
                            min="0"
                  ...
                  <td className="py-2">
                    {lines[index]?.itemId ? (
                      <>
                        ...
                        <span className="text-sm text-muted-foreground">From item</span>
                      </>
                    ) : (
                      <select
                        aria-label={`Line ${index + 1} expense account`}
                        className="flex h-9 w-full items-center justify-between rounded-md border border-input bg-transparent px-3 py-2 text-sm shadow-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring disabled:cursor-not-allowed disabled:opacity-50"
                  ...
                  {!amountsIncludeTax && (
                    <td className="money text-right">
                      {formatMoney(previews[index]?.netMinor ?? 0, currencyCode, minorUnit)}
                    </td>
                  )}
                  {!amountsIncludeTax && (
                    <td className="money text-right">
                      {formatMoney(previews[index]?.taxMinor ?? 0, currencyCode, minorUnit)}
                    </td>
                  )}

                  <td className="money text-right font-medium">
                    {formatMoney(previews[index]?.grossMinor ?? 0, currencyCode, minorUnit)}
                  </td>
                  <td>
                    <Button
                      type="button"
        ...
        <div className="mt-5 flex flex-col md:flex-row items-start justify-between gap-6">
          <div className="flex flex-col gap-3 pt-2">
            <label className="flex items-center gap-2 text-sm cursor-pointer">
              <input
                type="checkbox"
                className="size-4 rounded-[4px] border-border-strong accent-primary"
      ...
      <DocumentFormFooter onCancel={() => router.push(cancelHref)}>
        {status === "sent" ? (
          <Button type="button" disabled={isSubmitting} onClick={handleSubmit((values) => save(values, "issue"))}>
            {isSubmitting && <LoaderCircle className="size-4 animate-spin" />} Update Issued Quote
          </Button>
        ) : (
          <div className="flex gap-2">
            ...
            <Button type="button" disabled={isSubmitting} onClick={handleSubmit((values) => save(values, "issue"))}>
              {isSubmitting && <LoaderCircle className="size-4 animate-spin" />} Issue Quote
            </Button>
          </div>
        )}
      </DocumentFormFooter>
