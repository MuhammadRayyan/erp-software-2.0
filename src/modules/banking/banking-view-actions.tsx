...
export function BankingDocumentActions({ businessId, kind, id, number, status, journalId }: { businessId: string; kind: "transaction" | "transfer"; id: string; number: string; status: "draft" | "posted" | "void"; journalId?: string | null }) {
  ...
  return <><div className="flex flex-wrap gap-2">{kind === "transaction" && status === "draft" && <Button asChild><Link href={`/b/${businessId}/banking/transactions/${id}/edit`}><Pencil className="size-4" /> Edit Draft</Link></Button>}<DropdownMenu><DropdownMenuTrigger asChild><Button variant="secondary" aria-label="More actions">More <MoreHorizontal className="size-4" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end">{journalId && <DropdownMenuItem asChild><Link href={`/b/${businessId}/accounting/journal/${journalId}`}><BookOpenText className="size-4" /> View Journal Entry</Link></DropdownMenuItem>}{status === "posted" && <DropdownMenuItem className="text-danger focus:text-danger" onSelect={() => setConfirm(true)}><Ban className="size-4" /> Void</DropdownMenuItem>}</DropdownMenuContent></DropdownMenu></div><DialogRoot open={confirm} onOpenChange={setConfirm}><DialogContent><DialogTitle>Void {number}?</DialogTitle><DialogDescription>The ledger impact will be reversed. A completed reconciliation blocks this action.</DialogDescription>{error && <FormError message={error} />}<div className="mt-5 flex justify-end gap-2"><Button variant="secondary" onClick={() => setConfirm(false)}>Cancel</Button><Button variant="danger" disabled={pending} onClick={run}>Void document</Button></div></DialogContent></DialogRoot></>;
}
