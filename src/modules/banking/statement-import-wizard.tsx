...
export function StatementImportWizard({ businessId, accountId, accountName }: { businessId: string; accountId: string; accountName: string }) {
  ...
  return <div>
    ...
    {error && <div role="alert" className="mb-5 rounded-md border border-danger/25 bg-danger/10 px-3 py-2.5 text-sm text-danger">{error}</div>}
    ...
    ] as const).map(([key, label, required]) => <div key={key} className="space-y-1.5"><Label htmlFor={key}>{label}{!required && <span className="font-normal text-muted-foreground"> (optional)</span>}</Label><select id={key} className={selectClass} value={mapping[key]} onChange={(event) => setMapping((current) => ({ ...current, [key]: event.target.value }))}><option value="">Not mapped</option>{parsed.headers.map((header) => <option key={header} value={header}>{header}</option>)}</select></div>)}</div><div className="mt-7 flex justify-between"><Button variant="ghost" onClick={() => setStep(1)}><ArrowLeft className="size-4" /> Back</Button><Button onClick={continueToPreview}>Preview <ArrowRight className="size-4" /></Button></div></section>}
