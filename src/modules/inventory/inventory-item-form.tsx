...
export function InventoryItemForm({ businessId, itemId, salesAccounts, assetAccounts, expenseAccounts, initial }: { businessId: string; itemId?: string; salesAccounts: AccountOption[]; assetAccounts: AccountOption[]; expenseAccounts: AccountOption[]; initial: InventoryItemInput }) {
  ...
  return <form onSubmit={handleSubmit(save)} className="space-y-7" noValidate>
    ...
    <section className="border-b border-border pb-7"><h2 className="text-base font-semibold">Item details</h2><p className="mt-1 text-sm text-muted-foreground">SKU is optional but unique. Quantities are tracked separately at each location.</p><div className="mt-5 grid gap-5 md:grid-cols-3">
      ...
      <div className="space-y-1.5 md:col-span-3"><Label htmlFor="description">Description <span className="font-normal text-muted-foreground">(optional)</span></Label><textarea id="description" rows={3} className={selectClass + " h-auto py-2"} {...register("description")} /></div>
    </div></section>
    <section className="border-b border-border pb-7"><h2 className="text-base font-semibold">Accounting</h2><p className="mt-1 text-sm text-muted-foreground">Invoices use Sales and Inventory Asset; Delivery Notes use Cost of Sales and Inventory Asset.</p><div className="mt-5 grid gap-5 md:grid-cols-3">
      <div className="space-y-1.5"><Label htmlFor="salesAccountId">Sales account</Label><select id="salesAccountId" className={selectClass} {...register("salesAccountId")}>{salesAccounts.map((a) => <option key={a.id} value={a.id}>{a.code} {a.name}</option>)}</select></div>
      <div className="space-y-1.5"><Label htmlFor="inventoryAssetAccountId">Inventory Asset</Label><select id="inventoryAssetAccountId" className={selectClass} {...register("inventoryAssetAccountId")}>{assetAccounts.map((a) => <option key={a.id} value={a.id}>{a.code} {a.name}</option>)}</select></div>
      <div className="space-y-1.5"><Label htmlFor="costOfSalesAccountId">Cost of Sales</Label><select id="costOfSalesAccountId" className={selectClass} {...register("costOfSalesAccountId")}>{expenseAccounts.map((a) => <option key={a.id} value={a.id}>{a.code} {a.name}</option>)}</select></div>
    </div></section>
