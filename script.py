...
pattern = re.compile(r"\{error && \(\s*<div role=\"alert\" className=\"rounded-md border border-danger/25 bg-danger/10 px-3 py-2\.5 text-sm text-danger\">\s*\{error\}\s*</div>\s*\)\}")
    ...
    for file_name in files:
        if file_name.endswith(".tsx"):
            ...
            if pattern.search(content):
                content = pattern.sub("<FormError message={error} />", content)
