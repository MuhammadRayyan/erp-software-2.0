...
pattern = re.compile(r"<div className=\"sticky bottom-0 z-20 -mx-4 flex items-center justify-between gap-3 border-t border-border bg-surface/95 px-4 py-3 backdrop-blur-sm sm:mx-0 sm:rounded-t-lg sm:border-x\">\s*<Button (?:asChild )?variant=\"ghost\"><Link href=\{([^}]+)\}>Cancel</Link></Button>\s*(.*?)\s*</div>", re.DOTALL)
