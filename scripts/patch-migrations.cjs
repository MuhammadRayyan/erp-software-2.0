...
const migrationSql = `
function upgradeToPhase15(db: Database.Database) {
...
if (!content.includes("function upgradeToPhase15")) {
  content = content.replace(
    "export const businessMigrations: readonly SqliteMigration[] = [",
    migrationSql + "\nexport const businessMigrations: readonly SqliteMigration[] = ["
  );
  ...
  content = content.replace(
    ...
    `  {
    version: 15,
    name: "new_modules_and_discounts",
    up: upgradeToPhase15,
  },
] satisfies readonly SqliteMigration[];`
  );
  ...
  console.log("Migrations patched!");
} else {
