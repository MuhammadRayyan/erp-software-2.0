...
- The system schema is version `1`. Business migrations are ordered versions `0`-`11`: baseline, accounting, AR/AP, projects, inventory, banking, UAE VAT, outbound eInvoicing, inbound supplier eInvoicing, multi-currency foundation, document template settings, and customer active addresses. Each database has its own `schema_migrations`; pending migrations run once in individual immediate transactions and perform foreign-key checks.
