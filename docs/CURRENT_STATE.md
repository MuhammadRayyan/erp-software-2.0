...
- Master data/settings: customers and suppliers with legal, tax, registration, electronic-address, registered-address identity fields, and optional default currencies; business-local currency master/rates; realized FX account mappings; Chart of Accounts; accounting mappings; tax codes; document numbering; Form Defaults with per-document toggle configuration; UAE VAT registration; and a @react-pdf/renderer and Puppeteer-based document-template designer.
- Sales/AR: base- or foreign-currency non-posting Sales Quotes with full Revision/Versioning system (Rev 0, Rev 1 with switcher UI and superseded status), non-posting Sales Orders with 1-click quote-to-order conversion, draft/posted/void Sales Invoices with 1-click order-to-invoice conversion, same-currency Receipts with base Bank/Cash postings and allocations/reversals, inherited-rate Sales Credit Notes and allocations, multi-currency customer statements, base-carrying AR ageing, and PDFs.
- Purchases/AP: base- or foreign-currency non-posting Purchase Quotes with full Revision/Versioning system (Rev 0, Rev 1 with switcher UI and superseded status) and 1-click conversion to Purchase Orders; non-posting Purchase Orders with 1-click PO-to-invoice conversion, draft/posted/void Purchase Invoices, Debit Notes, same-currency Supplier Payments with base Bank/Cash postings and reversals, multi-currency supplier statements, base-carrying AP ageing, and PDFs.
...
- **Quote Revisions & Versioning (Sales & Purchase)**: Complete family tracking via `root_quote_id`, `base_quote_number`, `revision_number`, and `is_latest_revision` across both Sales Quotes and Purchase Quotes. Features sequential numbering (`SQ-0001-R1`, `PQ-0001-R1`), dropdown version switcher, warning banner on superseded iterations, and automatic status transitions.
- **One-Click Document Conversions**: Direct conversion actions with zero overhead: Purchase Quotes $\rightarrow$ Purchase Orders (status $\rightarrow$ `accepted`), Sales Quotes $\rightarrow$ Sales Orders (status $\rightarrow$ `accepted`), Sales Orders $\rightarrow$ Sales Invoices (status $\rightarrow$ `completed`), Purchase Orders $\rightarrow$ Purchase Invoices (status $\rightarrow$ `closed`).
- **Line Calculations & Tax Inclusivity**: All commercial forms (Invoices, Credit Notes, Purchase Orders, Purchase Quotes, etc.) support Manager.io-style line-level calculations via the `amountsIncludeTax` flag, dynamically extracting tax vs adding it on top. Line-level percentage and fixed-amount discounts are fully calculated and preserved across conversions.
- **Form UI & Defaults**: Full-width spreadsheet-like UI across all commercial document types. Configurable per-business Form Defaults (`/settings/form-defaults/[formId]`) for `amountsIncludeTax`, `showDiscounts`, `showLineNumber`, `showDescription`, and default notes/terms.
- **PDF Configuration**: React-PDF configurations track `headerImageUrl` and `footerImageUrl` uploads. Core generic templates (classic and modern) parse document lines and render Discount, Line Number, and Tax columns mapped back to DB records.
...
- The system schema is version `3` (`phase_0_system_schema`, `phase_1_user_settings`, `user_preferences`). Business migrations are ordered versions `0`-`18`: baseline, accounting, AR/AP, projects, inventory, banking, UAE VAT, outbound eInvoicing, inbound supplier eInvoicing, multi-currency foundation, document template settings, customer active addresses, numbering padding separation plus customer status cleanup, custom field definitions/values, sent emails, new modules and discounts, form defaults, sales invoice quote link, and sales quote revisions. Each database has its own `schema_migrations`; pending migrations run once in individual immediate transactions and perform foreign-key checks.
...
## Centralized Document Revision Engine (Latest Updates)
- Designed and implemented a robust, shared `RevisionEngine` in `src/core/versioning/revision-engine.ts` utilizing SQLite atomic transactions.
- Migrated Sales Quotes, Purchase Quotes, Sales Orders, and Purchase Orders to use the new Revision Engine.
- Achieved consistent, duplicate-free codebase for versioning documents that safely handles Drizzle validation, foreign key constraints (copying lines and custom fields), and strict draft concurrency checks.
