...
pattern = re.compile(r"<div className=\"sticky bottom-0 z-20 -mx-4 flex [^\"]*justify-between[^\"]*border-t border-border bg-surface/95 px-4 py-3 backdrop-blur-sm sm:mx-0 sm:rounded-t-lg sm:border-x\">\s*<Button (?:asChild )?variant=\"ghost\">\s*<Link href=\{([^}]+)\}>Cancel</Link>\s*</Button>\s*(.*?)\s*</div>", re.DOTALL)
