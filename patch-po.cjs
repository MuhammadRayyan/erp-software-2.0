const fs = require('fs');
let content = fs.readFileSync('src/modules/purchase-orders/purchase-order-form.tsx', 'utf-8');
...
if (myStart !== -1 && myEndMatch) {
  ...
  fs.writeFileSync('src/modules/purchase-orders/purchase-order-form.tsx', content);
